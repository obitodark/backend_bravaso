...
from app.schemas.holidays_schema import HolidaysResponseSchema, HolidaysDeliveryDatesResponseSchema
...
class HolidaysController:
    def __init__(self):
        ...
        self.schema_delivery = HolidaysDeliveryDatesResponseSchema

        self.__max_count = 5
        self.__start_hour = "07:00:00"
    ...
    def getById(self, id):
        try:
            ...
            return {
                'message': 'No se encontro el feriado mencionada'
            }, 404
        except Exception as e:
            return {
                'message': 'Ocurrio un error',
    ...
    def create(self, data):
        try:
            ...
            return {
                'messsage': 'El feriado se creo con exito',
    ...
    def update(self, id, data):
        try:
            if record := self.model.where(id=id).first():
                ...
                response = self.schema(many=False)
                return {
                    'messsage': 'El feriado se ha actualizado con exito',
                    'data': response.dump(record)
                }, 200
            return {
                'message': 'No se encontro el feriado mencionada'
        ...
        try:
            if record := self.model.where(id=id).first():
                if record.status:
                    ...
                    db.session.commit()
                return {
                    'message': 'Se deshabilito el feriado con exito'
                }, 200
            return {
                'message': 'No se encontro el feriado mencionada'
            }, 404
